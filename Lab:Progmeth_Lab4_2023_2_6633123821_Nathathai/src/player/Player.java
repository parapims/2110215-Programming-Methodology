...
public class Player {
    ...
    public String useWeapon(BaseWeapon item, Player opponent) {
        if (item.getRange() >= opponent.getPos()) {
    ...
    public void decreaseArmorDur() {
        if(helmetSlot.getDurability()-1<=0) helmetSlot = null;
        else helmetSlot.setDurability(helmetSlot.getDurability()-1);

        if(suitSlot.getDurability()-1<=0) suitSlot = null;
        else suitSlot.setDurability(suitSlot.getDurability()-1);

        if(bootsSlot.getDurability()-1<=0) bootsSlot = null;
        else bootsSlot.setDurability(bootsSlot.getDurability()-1);
    }
    ...
    public void decreaseWeaponDur(BaseWeapon item) {
        ...
        else item.setDurability(item.getDurability()-1);
    }
    ...
    public String useNonWeaponItem(BaseItem item) {
        ...
        if(item instanceof Healable){
            Healable healable = (Healable) item;
            s.add(useHeal(healable));
        }
        if(item instanceof AttBuffable){
            AttBuffable attbuffable = (AttBuffable) item;
            s.add(useBuff(attbuffable));
        }
        INVENTORY.removeItem(item);
